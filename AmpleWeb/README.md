...
2.  **Manual Start (For Developers)**:
    ...
    *   **(Optional)** To rebuild `public/wasm/mame.wasm.gz` (currently powered by a custom MAME 0.289-patched core), use the [MameWasm](https://github.com/anomixer/MameWasm) project to build the binary, then compress it to `.gz`.
