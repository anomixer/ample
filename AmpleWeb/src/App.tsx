...
/**
 * Download a disk image from a URL, falling back to the CORS proxy, and return
 * it as a File (ZIPs are unzipped to their first disk image via `unzip`). Returns null on failure.
 */
async function downloadMediaFromUrl(
  url: string,
  unzip: (file: File) => Promise<File>,
  log?: (msg: string, isError: boolean) => void,
): Promise<File | null> {
  const say = (msg: string, isError = false) => log?.(msg, isError)
  let resp: Response | null = null
  try {
    resp = await fetch(url)
  } catch { /* network/CORS error — try proxy */ }

  if (!resp || !resp.ok) {
    say(`Direct fetch failed (CORS?), trying proxy...`)
    try {
      resp = await fetch(`${CORS_PROXY}${url}`)
    } catch { resp = null }
  }
...
/**
 * Fetch a ROM zip from the local `/roms` directory, falling back to the configured
 * auto-download servers (in order). Returns the ROM file, or null if unavailable.
 */
async function fetchRomWithFallback(
  filename: string,
  driver: string,
  romSettings: { autoDownload: boolean; downloadServers: string[] },
  log?: (msg: string, isError: boolean) => void,
): Promise<RomFile | null> {
  try {
    return await fetchRom(`${BASE_URL}roms/${filename}`, driver, filename)
  } catch { /* try download servers */ }

  if (romSettings.autoDownload && romSettings.downloadServers.length > 0) {
    for (const server of romSettings.downloadServers) {
      const downloadUrl = server.replace('{filename}', filename)
      log?.(`Attempting download: ${downloadUrl}`, false)
      try {
        const rom = await fetchRom(downloadUrl, driver, filename)
        log?.(`Downloaded: ${filename} from ${server}`, false)
        return rom
      } catch { continue }
    }
  }
  log?.(`ROM not found: ${filename}`, true)
  return null
}
...
function App() {
  ...
  const fetchAllRoms = useCallback(async (machineName: string, effectiveDriver: string, configOverride?: MachineConfig | null, slotsOverride?: Record<string, string>): Promise<RomFile[]> => {
    ...
    for (const romFile of romFilesToFetch) {
      ...
      const rom = await fetchRomWithFallback(romFile, effectiveDriver, romSettings, addLog)
      if (!rom) continue

      // TorrentZip check — strip the 40-byte footer so MAME's ZIP parser can read it.
      // No-op unless the footer is the torrentzip signature, so it's safe to apply to
      // both local and server-downloaded ROMs.
    ...
    if (currentConfig) {
      ...
      for (const devName of finalDeviceList) {
        ...
        // Slot ROMs are optional peripherals — log misses as non-error notes.
        const slotLog = (msg: string, isError: boolean) =>
          addLog(isError ? msg.replace(/^ROM not found: /, 'Slot ROM not found: ') : msg, false)
        const rom = await fetchRomWithFallback(`${devName}.zip`, effectiveDriver, romSettings, slotLog)
        if (rom) {
          romFiles.push(rom)
  ...
  const handleZipFile = async (file: File): Promise<File> => {
    ...
    try {
      ...
      // Object holder avoids TS's control-flow quirk of narrowing a closure-mutated `let`
      // to `never`/unassigned. The object is always initialized; its optional props hold results.
      const found: { file?: JSZip.JSZipObject; name?: string } = {}

      zip.forEach((relativePath: string, entry: JSZip.JSZipObject) => {
        if (!found.file && !entry.dir) {
          const lower = relativePath.toLowerCase()
          if (diskExts.some(ext => lower.endsWith(ext))) {
            found.file = entry
            found.name = relativePath.split('/').pop() || 'disk.dsk'
          }
        }
      })

      if (found.file) {
        const content = await found.file.async('blob')
        const diskName = found.name ?? 'disk.dsk'
  ...
  const handleInsertUrl = useCallback(async (id: string) => {
    ...
    try {
      ...
      const file = await downloadMediaFromUrl(url, handleZipFile, addLog)
            ...
            if (id && nameOrUrl) {
              if (nameOrUrl.startsWith('http')) {
                ...
                try {
                  ...
                  const file = await downloadMediaFromUrl(nameOrUrl, handleZipFile, addLog)
                  if (!file) throw new Error('Download failed')
                  restoredMedia[id] = file
                  await dataManager.saveMedia(id, file)
                  addLog(`Downloaded and saved: ${file.name}`, false)
